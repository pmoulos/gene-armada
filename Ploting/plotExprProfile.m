function hp = plotExprProfile(x,y,varargin)
...
set(gca,'FontSize',8,'FontWeight','bold','XLim',xlims,'XTick',x)
xlabel('Conditions','FontSize',10,'FontWeight','bold');
ylabel('Expression','FontSize',10,'FontWeight','bold');
if ~isempty(titre)
...
% % Make the markers solid
% for i=1:length(h)
%     mcol=get(h(i),'Color');
%     set(h(i),'MarkerFaceColor',mcol)
% end
...
if leg && ~isempty(labels)
    legend(h,labels,'FontSize',8,'FontWeight','bold')
end
