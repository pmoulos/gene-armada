function [hOut,hLines] = mymaimage3D(maStruct,attrib,field,varargin)
...
hImage = surf(theData(attrib.Indices),'Parent',ax,'FaceColor','interp',...
                                        'LineStyle','none');
axis vis3d
...
for count = 2:numXBreaks
    hLines(count-1) = line([XBreaks(count),XBreaks(count)]-.5,[0,maxY]+.5,'linewidth',1,'color','k');
end
for count = 2:numYBreaks
    hLines(numXBreaks+count-2) = line([0,maxX]+.5,[YBreaks(count),YBreaks(count)]-.5,'linewidth',1,'color','k');
end
% turn off axis
axis('off');
