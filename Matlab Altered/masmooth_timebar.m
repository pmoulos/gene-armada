function c = masmooth_timebar(x,y,span,method,iter,weighting)
...
% 
% References: 
% [1] Bowman and Azzalini 
%     "Applied Smoothing Tehcniques for Data Analysis" 
%     Oxford Science Publications, 1997
% [2] Orfanidis, S.J.
%     "Introduction to Signal Processing",
%     Prentice-Hall, Englewood Cliffs, NJ, 1996.
%
% See Curvefitting Toolbox for more smoothing options.

%   Copyright 2001-2004 The MathWorks, Inc. 
%   $Revision: 1.2.4.5 $  $Date: 2004/12/24 20:44:56 $

global HH           % External handle for global timebar
global TOTAL_GOOD   % Help variable for global timebar
...
if nargin<4 || isempty(method)
    ...
    switch method
        ...
        case 'sgolay'
            runSgolay = true;
        otherwise
            error('Bioinfo:MAsmoothUnknownMethod',...
...
if runSgolay
    if nargin>5
        error('Bioinfo:TooManyInputsForSgolay',...
              'Too many input arguments for SGOLAY method.')
    end
...
runFAlg = ~repeatedX && ~yNaNs && (span>4);
  ...
  % of the sliding window changes), 2) does not support NaNs in y
...
runSFAlg = runFAlg && (xIsOwnEnumeration||((max(diffX)-min(diffX))<(max(x(xnotNaN))*eps)));
  % the super fast alg. is only for all above + equally distributed samples
...
if (weighting == 2) && (span-1 <= order)
    span = order+2;
    warning('Bioinfo:SmallSpan',...
      'Rank deficiency when SPAN-1 <= ORDER, changing SPAN to %d.',span)    
elseif span-2 <= order
    span = order+3;
    warning('Bioinfo:SmallSpan',...
      'Rank deficiency when SPAN-2 <= ORDER, changing SPAN to %d.',span)
end
...
if robust>0
    ...
    rbound = zeros(n,1);
    dmaxv = zeros(n,1);
    timebar_freq=(robust+1)*n; %adjust the time window for the timebar depending if the method is robust or not
else
    timebar_freq=n;%adjust the time window for the timebar when not robust
end
...
if robust>0
    h = timebar('Robust Lowess normalization','Progress');
else
    h = timebar('Lowess normalization','Progress');%initialize timebar
end
...
for i=1:n
    ...
    COU=COU+1;
    timebar(h,i/timebar_freq)%added time bar to monitor data processing
    timebar(HH,COU/(TOTAL_GOOD*(robust+1)));
...
for k = 1:robust
    ...
    for i=1:n
        ...
        %COU=COU+n+1;
        COU=COU+1;
        timebar(h,(n+jj)/timebar_freq);%added time bar to monitor data processing
        timebar(HH,COU/(TOTAL_GOOD*(robust+1)));
        if theDiffs(i) == 0
            c(i) = c(i-1);
            continue;
...
if robust>0
    timebar_freq=(robust+1)*n; %adjust the time window for the timebar depending if the method is robust or not
    h = timebar('Robust Fast Lowess normalization','Progress');%initialize robust timebar
else
    timebar_freq=n;%adjust the time window for the timebar when not robust
    h = timebar('Fast Lowess normalization','Progress');%initialize timebar
end 
...
for j=1:halfw
     ...
     %timebar(h,j/timebar_freq)%added time bar to monitor data processing
