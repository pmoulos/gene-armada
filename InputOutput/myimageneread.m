function output = myimageneread(filename,varargin)
...
%           Indices
%           Shape
%           Control (a possible field if the raw arithmetic data columns contain text, 
%           added by P.M.)
%
...
% Modified on 2006/08/26 by Panagiotis Moulos. Problems reading the 'Control' Field
% and the rest of Data fixed.
% Latest update by P.M. on 2007/02/26
...
% (P.M.) and check if there is possibly a text column among the raw data (after ImaGene 5.0)
z=strmatch('Control',colNames,'exact');
if ~isempty(z)
    format = ['%s%s%f%f%f%f%s' repmat('%f',1,z-7) '%s' repmat('%f',1,numFields-z)];
else
    format = ['%s%s%f%f%f%f%s' repmat('%f',1,numFields-6)];
end
rawdata = textscan(fid,format,'HeaderLines',EndHeader+2,'Delimiter','\t');
rawdata(1) = [];
...
try
    % Because the Field 'Control' contains text
    if ~isempty(z)
        part1 = cell2mat(rawdata(7:z-1));
        part2 = cell2mat(rawdata(z+1:end));
        output.Data = [part1 part2];
    else
...
if ~isempty(z)
    ctrlToEnd = [7:z-1 z+1:length(colNames)];
    output.ColumnNames = colNames(ctrlToEnd);
    output.Control = rawdata{z};
else
