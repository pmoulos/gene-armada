function exportDEfinal(exprp,exptab,DataCellNormLo,DataCellStat,fcinds,opts,filename)
...
if strcmpi(opts.outtype,'text') % Text tab delimited files
    ...
    if opts.foldchange && length(DataCellStat)>8 && ~isempty(fcinds)
        for j=1:length(fcinds{1})
            ...
            evaltext=[evaltext,'DataCellStat{9}(currentIndex,',num2str(j),'),',...
                               '2.^DataCellStat{9}(currentIndex,',num2str(j),'),'];
        end
    end
...
elseif strcmpi(opts.outtype,'excel') % Excel files
    ...
    for i=1:t
        ...
        % Normalized Raw Ratios
        if opts.normrawratio && length(DataCellStat{6})==t
            for j=1:length(exprp{i})
