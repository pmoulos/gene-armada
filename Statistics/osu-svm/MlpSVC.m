function [AlphaY, SVs, Bias, Parameters, nSV, nLabel] = MlpSVC(Samples, Labels, Gamma, Coeff, C)
% USAGES: 
...
%    [AlphaY, SVs, Bias, Parameters, nSV, nLabel] = RbfSVC(Samples, Labels, Gamma,Coeff, C)
%
% DESCRIPTION: 
%   Construct a non-linear SVM classifier with a sigmoid (multilayer perceptron)  kernel, 
...
%            of (tanh(Gamma*<X(:,i)-X(:,j)>+Coefficient)). (default 1)
%   Coeff: The ceofficient above
...
%
% By Panagiotis Moulos (03/24/2008)
%
...
if (nargin < 2) & (nargin > 5)
   ...
   elseif  (nargin == 3)
       Parameters = [2 1 Gamma];
       [AlphaY, SVs, Bias, Parameters, nSV, nLabel] = SVMTrain(Samples, Labels, Parameters);
   elseif  (nargin == 4)
       Parameters = [2 1 Gamma Coeff];
       [AlphaY, SVs, Bias, Parameters, nSV, nLabel] = SVMTrain(Samples, Labels, Parameters);
    elseif  (nargin == 5)
       Parameters = [2 1 Gamma Coeff C];
       [AlphaY, SVs, Bias, Parameters, nSV, nLabel] = SVMTrain(Samples, Labels, Parameters);
   end
end
