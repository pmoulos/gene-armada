function outPoints = goodEachCond(datstruct,BGBadpoints,BelowBackgroundPoints,conds,auto)
...
for d=1:max(size(datstruct))
    outBGGoodPointsSliPosTemp{d}=BGGoodPointsSliPosTemp{d}{1};
    for i=2:max(size(BGGoodPointsSliPosTemp{d}))
        outBGGoodPointsSliPosTemp{d}=[outBGGoodPointsSliPosTemp{d};BGGoodPointsSliPosTemp{d}{i}];
    end
    outBGGoodPointsSliPosTemp{d}=unique(outBGGoodPointsSliPosTemp{d});
...
if ~auto
    ...
    nam=strcat(nam,'_','GoodEachCondition');
