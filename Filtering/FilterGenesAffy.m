function [DataCellNormLo,TotalBadpoints] = FilterGenesAffy(datstruct,DataCellNormLo,cdffile,varargin)
...
usewaitbar=false;                  % Use waitbars for MAS5 calls and reproducibility test
htext=[];                          % Empty handle
...
if length(varargin)>1
    ...
    okargs={'mas5calls','marginasabsent','iqr','variance','intensity','custom','reptest',...
            'pval','showhist','exportfilt','conditions','usewaitbar','htext'};
            ...
            switch(j)
                case 1 % MAS5 Calls options
                    if isempty(cell2mat(parVal))
                        ...
                        if iscell(parVal) && length(parVal)~=3
                            error('The %s parameter value must be a cell of length 3',parName)
                        end
                        if iscell(parVal)
                            if ~isscalar(parVal{1}) || ~isnumeric(parVal{1}) || parVal{1}<0 || parVal{1}>1
                                error('The 1st part of %s parameter value must be a number between 0 and 1',parName)
                            end
                            if ~isscalar(parVal{2}) || ~isnumeric(parVal{2})
                                error('The 2nd part of %s parameter value must be a number between 0 and 1',parName)
                            end
                            if ~isvector(parVal{3}) || ~isnumeric(parVal{3}) || parVal{3}(1)<0 || parVal{3}(1)>1 ...
                                    || parVal{3}(2)<0 || parVal{3}(2)>1
                                error('The 3rd part of %s parameter value must be a vector of 2 numbers between 0 and 1',parName)
                            end
                        end
                        mas5opts=parVal;
                    end
                ...
                case 11 % Condition names for exporting
                    ...
                    condnames=parVal;
                case 12 % Show reproducibility test p-value histograms
                    if ~islogical(parVal)
                        error('The %s parameter value must be a true or false',parName)
                    end
                    usewaitbar=parVal;
...
if ~isempty(custfilt) % Run only this
    ...
    rexp='[\s\(\|&\+-\*\\]*(PA|PT|PL|PU)+\s*=\s*(((\+|-)?[0-9]+\.[0-9]*)|(\+?[0-9]+))[\s\)\|&\+-\*\\]*';
    ...
    for i=1:length(args)
        ...
        switch args{i}(1:2)
            case 'PA'
                mas5opts{1}=str2double(args{i}(k+1:end));
                cal=true;
            case 'PT'
                mas5opts{2}=str2double(args{i}(k+1:end));
                cal=true;
            case 'PL'
                mas5opts{3}(1)=str2double(args{i}(k+1:end));
                cal=true;
            case 'PU'
                mas5opts{3}(2)=str2double(args{i}(k+1:end));
                cal=true;
            case 'IN'
        ...
        if margasabs
            for i=1:length(datstruct)
                for j=1:length(datstruct{i})
                    mas5bad{i}{j}=union(strmatch('A',calls{i}{j}),strmatch('M',calls{i}{j}));
                end
            end
        else
            for i=1:length(datstruct)
                for j=1:length(datstruct{i})
                    mas5bad{i}{j}=strmatch('A',calls{i}{j});
                end
            end
        end
    ...
    for i=1:length(datstruct)
        for j=1:length(datstruct{i})
            TotalBadpoints{i}{j}=union(mas5bad{i}{j},rest{i}{j});
        end
    end
    ...
    [TotalBadpoints,mas5bad,iqrbad,varbad,intenbad]=initVars(datstruct,mas5opts,iqrpct,varpct,intencut);
...
%     else % Assign all present (we are going to need that on exporting
%         
%         l=length(datstruct);
%         calls=cell(1,l);
%         for i=1:l
%             calls{i}=cell(1,length(datstruct{i}));
%             for j=1:length(length(datstruct{i}))
%                 calls{i}{j}=repmat({'P'},[length(DataCellNormLo{1}{1}{1}),1]);
%             end
%         end
%         DataCellNormLo{6}=calls;
    ...
    for i=1:length(datstruct)
        for j=1:length(datstruct{i})
            TotalBadpoints{i}{j}=union(mas5bad{i}{j},union(intenbad{i}{j},union(iqrbad,varbad)));
        end
    end
...
if ~strcmpi(reptest,'none')
    repbadp=reprodTest(condnames,DataCellNormLo{2},reptest,pval,DataCellNormLo{5}{4},...
                       dishis,htext);
    for i=1:length(datstruct)
        for j=1:length(datstruct{i})
...
function [TB,mb,ib,vb,inb]=initVars(stru,mo,ip,vp,ic)

TB=cell(1,length(stru));
ib=[];
vb=[];
mb=cell(1,length(stru));
for i=1:length(stru)
    mb{i}=cell(1,length(stru{i}));
end
