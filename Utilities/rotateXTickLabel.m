function hText = rotateXTickLabel(XTick,rot,varargin)
%hText = rotateXTickLabel(XTick,rot,XTickLabel,varargin)     Rotate XTickLabel
%
% Syntax: rotateXTickLabel
%
% Input:    
...
% font size
%    rotateXTickLabel([],45,[],'Fontsize',14)
%
% Example 3:  Set the positions of the XTicks and rotate them 90°
%    figure;  plot([1960:2004],randn(45,1)); xlim([1960 2004]);
%    rotateXTickLabel([1960:2:2004]);
%
% Example 4:  Use text labels at XTick positions rotated 45° without tex interpreter
%    rotateXTickLabel(XTick,45,NameFields,'interpreter','none');
%
% Example 5:  Use text labels rotated 90° at current positions
%    rotateXTickLabel([],90,NameFields);
%
% Note : you can not re-run rotateXTickLabel on the same graph. 
%
% 


% This is a modified version of rotateXTickLabel90 by Denis Gilbert
...
%
% See also: rotateXTickLabel90, TEXT,  SET

% Based on rotateXTickLabel90
%   Author: Denis Gilbert, Ph.D., physical oceanography
%   Maurice Lamontagne Institute, Dept. of Fisheries and Oceans Canada
%   email: gilbertd@dfo-mpo.gc.ca  Web: http://www.qc.dfo-mpo.gc.ca/iml/
%   February 1998; Last revision: 24-Mar-2003

% check to see if rotateXTickLabel has already been here (no other reason for this to happen)
if isempty(get(gca,'XTickLabel')),
    error('rotateXTickLabel : can not process, either rotateXTickLabel has already been run or XTickLabel field has been erased')  ;
end
...
if length(XTick) ~= length(xTickLabels),
    error('rotateXTickLabel : must have same number of elements in "XTick" and "XTickLabel"')  ;
end
