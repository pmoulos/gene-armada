function h = timebar(message,name,x,y,update_rate)
...
%    returned.
%
%    TIMEBAR(H,F) will update the length of the progress bar, the
%    percentage complete, and the estimated time remaining, where
%    F is a fractional progress between 0 (initial) and 1 (complete).
%    (Note that the order of H,X is opposite to waitbar.)
%
%    TIMEBAR(H,F,X,Y) will define the position of the timebar 
%    window. X and Y are values between 0 and 1 (added by PM)
%
%    TIMEBAR(H,F,X,Y,RATE) will update the progress bar and information
...
%    2.1   2006-10-3    Added user defined window screen coordinates (PM)
%
...
if nargin < 3                                               % If coordinates giver for timebar 
    x=0.5; y=0.5;                                           % window not given, initialize it
end                                                         % in the middle of the screen

if nargin < 4                                               % Same as before but if only the 
    y=0.5;                                                  % horizontal position is given
end

if nargin < 5                                               % If update rate is not input
    update_rate = 0.1;                                      % set rate to 0.1                                           % seconds and (x,y)=(0.5,0.5) (middle of screen)
end
...
if ~ishandle(message)                                       % If first input is not a timebar handle, 
    ...
    winpos = [x*(screenwidth-winwidth), ...
       y*(screenheight-winheight), winwidth, winheight];  % Position of timebar window origin
...
%%%%%%%% END TIMEBAR HANDLE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%function h = timebar(message,name,update_rate)
